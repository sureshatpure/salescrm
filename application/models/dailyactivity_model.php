        ...
        // echo $sql;   die;                
			...
			function fncheck_prodgroup_dup_saleorder($customergroup,$prodgroup)
			{
				...
				$sql1="SELECT 	leaddetails.leadid,leaddetails.leadid as id,sum(lead_prod_potential_types.potential)as potential
					...
					INNER JOIN view_tempitemmaster_grp ON view_tempitemmaster_grp.id=leadproducts.productid 
					INNER JOIN lead_prod_potential_types ON lead_prod_potential_types.leadid=leaddetails.leadid 
					WHERE replace(trim(customermasterhdr.customergroup),'''','')='".html_entity_decode($customergroup)."' AND trim(view_tempitemmaster_grp.itemgroup)='".$prodgroup."' AND leaddetails.lead_close_status=0 and converted=0 AND leaddetails.lead_close_status=0 and converted=0 	GROUP BY leaddetails.leadid";
			 ...
			 function check_saleorderentries($customergroup,$prodgroup)
			 {
			 	$customergroup=urldecode($customergroup);
				$prodgroup =urldecode($prodgroup);
			 ...
			 function check_leadduplicates($customergroup,$prodgroup)
			 {
			 	$customergroup=urldecode($customergroup);
				$prodgroup =urldecode($prodgroup);
                $sql="SELECT 	leaddetails.leadid,leaddetails.leadid as id,sum(lead_prod_potential_types.potential)as potential
                ...
                $result = $this->db->query($sql);
                          echo "in check_leadduplicates "; echo"<pre>";print_r($result);echo"</pre>";
                $rowcount = $result->num_rows();
                return $rowcount;
			 }
